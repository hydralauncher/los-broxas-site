...
// touch: nudge with device orientation when available
window.addEventListener("deviceorientation", (e) => {
  if (e.gamma == null || e.beta == null) return;
  onMove(
    ((Math.max(-30, Math.min(30, e.gamma)) / 30) + 1) / 2 * window.innerWidth,
    ((Math.max(-30, Math.min(30, e.beta - 30)) / 30) + 1) / 2 * window.innerHeight
  );
}, { passive: true });
